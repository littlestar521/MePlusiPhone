...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeLabelText:) name:@"login" object:nil];
    ...
    [messageView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"Bg_menu"]]];
//    messageView.backgroundColor = kMainColor;
    [self.setView addSubview:messageView];
    
    UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(60, 50, 80, 80)];
    imageView.image = [UIImage imageNamed:@"logo_small"];
...
//            self.number.backgroundColor = [UIColor yellowColor];
    ...
    for (int i = 0; i < 3; i++) {
        ...
        titleLable.textColor = kMainColor;
